...
SELECT EMP_ID, EMP_NAME, SALARY, SALARY * 12 AS YEARSALARY FROM EMPLOYEE;
...
-- 사번, 이름, 부서코드, 직급코드 조회
SELECT EMP_ID, EMP_NAME, SALARY, DEPT_CODE  FROM EMPLOYEE WHERE DEPT_CODE = 'D9'; 
...
-- EMPLOYEE 테이블에서 급여가 300만 이상 또는 500만 미만 이상인 사원의
-- 사번, 이름, 급여, 전화번호 조회
...
-- 300만 이상, 600만 이하
SELECT EMP_ID, EMP_NAME, SALARY FROM EMPLOYEE WHERE SALARY BETWEEN 3000000 AND 6000000;


-- NOT BETWEEN A AND B : A 이상 B 이하가 아닌 경우 --> A 미만, B 초과

SELECT EMP_ID, EMP_NAME, SALARY FROM EMPLOYEE WHERE SALARY NOT BETWEEN 3000000 AND 6000000;
...
-- 사번, 이름, 전화번호 조회
SELECT EMP_ID, EMP_NAME FROM EMPLOYEE WHERE PHONE LIKE '010%';
-- WHERE PHONE LIKE '010_________';



-- EMPLOEE 테이블에서 전화번호가 010으로 시작하지 않는 사원의 
-- 사번, 이름, 전화번호 조회
SELECT EMP_ID, EMP_NAME FROM EMPLOYEE WHERE PHONE NOT LIKE '010%';



-- EMAIL에서 _ 앞에 글자가 세 글자인 사원 이름, 이메일 조회
SELECT EMP_NAME, EMAIL FROM EMPLOYEE WHERE EMAIL LIKE '____%';
-- 문제점 : _를 기준점으로 삼았으나, 패턴 중 '_'와 동일한 표기법으로 작성되어 구분되지 않음
...
SELECT EMP_ID, EMP_NAME, EMAIL, DEPT_CODE, HIRE_DATE, SALARY FROM EMPLOYEE WHERE 
EMAIL LIKE '____#_%' ESCAPE '#' AND (DEPT_CODE = 'D9' OR DEPT_CODE = 'D6') AND HIRE_DATE BETWEEN '1990-01-01' AND '2000-12-31' AND SALARY >=2700000;
...
OR DEPT_CODE IS NULL; -- 부서 코드가 없는 2명 (NULL처리 연산자)

-- NOT IN만 쓰면 NULL인 사람들이 나오지 않는다 => IS NULL 같이 써줘야 함
...
-- 사번, 이름, 급여 조회
SELECT EMP_ID ,EMP_NAME, SALARY FROM EMPLOYEE ORDER BY SALARY; -- ASC가 기본값
...
-- SELECT절이 ORDER BY보다 해석 순서가 먼저라서 별칭이 먼저 지정되어있다 => 별칭 사용 가능해짐


/* 정렬중첩 : 대분류 정렬 후 소분류 정렬 */
