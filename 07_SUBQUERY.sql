...
SELECT EMP_NO, EMP_NAME, JOB_CODE, SALARY FROM EMPLOYEE WHERE SALARY >= 3047663;

-- 3) 전 직원의 평균 급여보다 많은 급여를 받고 있는 직원 조회
--> 위의 2단계를 하나의 쿼리로 만든다 --> 1번 쿼리를 서브쿼리로
SELECT EMP_NO, EMP_NAME, JOB_CODE, SALARY FROM EMPLOYEE
 ...
 * 
 * - 다중열 서브쿼리 : 서브쿼리의 SELECT절에 나열된 항목 수가 여러개일 때
 * 
...
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME, '사원' 구분 FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE)
...
-- 선택함수 사용!
--> DECODE(컬럼명, 값1, 1인경우, 값2, 2인 경우,..일치하지 않는 경우)
...
-- 2) 직급이 과장인 직원들 급여 조회
SELECT SALARY FROM EMPLOYEE NATURAL JOIN JOB WHERE JOB_NAME = '과장';
...
SELECT EMP_ID, EMP_NAME, JOB_NAME, SALARY FROM EMPLOYEE
NATURAL JOIN JOB 
WHERE SALARY > ANY (SELECT SALARY FROM EMPLOYEE NATURAL JOIN JOB WHERE JOB_NAME = '과장')
AND JOB_NAME = '대리';
...
WHERE DEPT_CODE IN (SELECT DEPT_ID FROM DEPARTMENT -- 다중행
									WHERE LOCATION_ID = (SELECT LOCAL_CODE FROM LOCATION WHERE NATIONAL_CODE = 'KO')); -- 단일행
...
WHERE (DEPT_CODE, JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE FROM EMPLOYEE WHERE SUBSTR(TO_CHAR(HIRE_DATE, 'YYYY'),1,4) = '2000');
 ...
 * --> 선동일은 JOB_CODE가 'J1'이고, SALARY가 8백만원
 * 
...
FROM EMPLOYEE MAIN
LEFT JOIN DEPARTMENT  ON (DEPT_ID = DEPT_CODE)							
WHERE MAIN.MANAGER_ID IN (SELECT EMP_ID FROM EMPLOYEE SUB
 ...
 * 
 * 2. 차태현 입사일이랑 다름. 차태연 아님! (RESULT SET 제외)
...
LEFT JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE)
WHERE ENT_YN = 'N'
AND HIRE_DATE = (SELECT MIN(HIRE_DATE) FROM EMPLOYEE SUB
...
SELECT EMP_NAME, JOB_CODE, SALARY, (SELECT AVG(SALARY) FROM EMPLOYEE ) FROM EMPLOYEE;
...
-- (스칼라 + 상관쿼리)
SELECT EMP_NAME, JOB_CODE, SALARY,
(SELECT CEIL(AVG(SALARY)) FROM EMPLOYEE SUB WHERE MAIN.JOB_CODE = SUB.JOB_CODE) "직급의 급여 평균"
FROM EMPLOYEE MAIN
ORDER BY JOB_CODE;
...
-- SELECT절에 없어도 WHERE절에서 ROWNUM 사용 가능
