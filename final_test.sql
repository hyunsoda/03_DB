...
ALTER TABLE "ITEM_IMG" ADD IMG_TYPE_NO NUMBER;
COMMIT;
...
COMMENT ON COLUMN "RENT_DETAIL"."RENT_DETAIL_NO" IS '대여 상세 번호(PK)';

COMMENT ON COLUMN "RENT_DETAIL"."RENT_NO" IS '대여 번호(FK)';

COMMENT ON COLUMN "RENT_DETAIL"."ITEM_NO" IS '상품 고유 번호(FK)';

COMMENT ON COLUMN "RENT_DETAIL"."CAR_STOCK_NO" IS '자동차 재고 번호(FK)';

COMMENT ON COLUMN "RENT_DETAIL"."RETURN_FL" IS '반납여부';
...
COMMENT ON COLUMN "RENT"."RENT_NO" IS '대여 번호(PK)';

COMMENT ON COLUMN "RENT"."RENT_DATE" IS '대여 예정 날짜';

COMMENT ON COLUMN "RENT"."EXPECT_DATE" IS '반납 예정 날짜';

COMMENT ON COLUMN "RENT"."RENT_DEL_FL" IS '대여 취소 여부 (Y/N)';

COMMENT ON COLUMN "RENT"."EXPIRE_DATE" IS '반납 날짜';

COMMENT ON COLUMN "RENT"."MEMBER_NO" IS '회원 번호(FK)';
...
SELECT * FROM "ITEM";
		...
		SELECT ITEM_NO, EQUIPMENT_NAME, TO_NUMBER(TO_CHAR(EQUIPMENT_RENT_PRICE,'FM999,999')), ITEM_VIEW_COUNT, EQUIPMENT_SELL_PRICE , EQUIPMENT_CATEGORY_NAME, ITEM_DETAIL_CONTENT,
			(SELECT IMG_PATH || IMG_RENAME
			 FROM ITEM_IMG
			 WHERE ITEM_NO =58
			 AND  IMG_ORDER = 0) THUMBNAIL
