...
* 사용자 계정 : 데이터베이스에 대하여 질의, 갱신, 보고서 작성 등의
                ...
                ex) kh_ash계정(각자 이니셜 계정), workbook 등
*/


-- GRANT RESOURCE, CONNECT TO kh_ash;
...
-- ROLE_SYS_PRIVS : 오라클 DB에서 시스템 권한을 가진 역할을 나타내는 데이터 딕셔너리뷰 
...
-- 2. 새 연결 추가 
-- 사용자 ahs_sample는 CREATE SESSION 권한을 가지고 있지 않음; 로그온이 거절되었습니다.
--> 접속 권한 (CREATE SESSSION) 없어어 오류 발생
...
ALTER USER ash_sample DEFAULT TABLESPACE 
...
-- kh_ash / ash_sample 사용자 계정끼리 서로 객체 접근 권한 부여


-- 1. (sample) kh_ash 계정의 EMPLOYEE 테이블 조회
SELECT * FROM kh_ash.EMPLOYEE;
...
GRANT SELECT ON EMPLOYEE TO ash_sample;
